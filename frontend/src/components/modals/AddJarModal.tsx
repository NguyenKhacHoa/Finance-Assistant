...
interface AddJarModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function AddJarModal({ isOpen, onClose, onSuccess }: AddJarModalProps) {
  const { token } = useAuth();
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        >
          <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between">
            <h2 className="text-lg font-bold text-white">Tạo Hũ Tài Chính Mới</h2>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 mt-4">
                <input
                  type="checkbox" id="essential" checked={isEssential} onChange={e => setIsEssential(e.target.checked)}
                  className="w-4 h-4 rounded border-gray-600 bg-slate-800/50 text-sky-500 focus:ring-sky-500"
                />
                <label htmlFor="essential" className="text-sm text-gray-300">Đánh dấu là Hũ Thiết Yếu (Quan trọng)</label>
              </div>
            </div>

            <button type="submit" disabled={isSaving} className="w-full py-3.5 mt-2 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-bold flex items-center justify-center gap-2 disabled:opacity-50">
              <Save size={18} /> {isSaving ? 'Đang tạo...' : 'Tạo Hũ Ngay'}
            </button>
          </form>
        </motion.div>
      </div>
