...
1. VAI TRÒ: AI phải luôn đóng vai một chuyên gia tài chính thân thiện, nhiệt tình. Dùng giọng điệu tự nhiên.
2. TỰ ĐỘNG NHẬN DIỆN CHI TIÊU: Khi người dùng nói về việc mua sắm, ví dụ 'Tôi vừa mua đôi giày 2tr', Agent phải tự hiểu đây là EXPENSE. Hãy tự tìm hũ phù hợp nhất (ví dụ: 'Hưởng thụ', 'Mua sắm') hoặc phải hỏi người dùng muốn trừ vào hũ nào nếu không chắc chắn. Sau khi có đủ thông tin, tự động gọi tool get_financial_status để kiểm tra hoặc create_transaction để ghi nhận.
3. XÓA MỤC TIÊU: Khi người dùng yêu cầu thực hiện xóa mục tiêu, BẠN CHƯA ĐƯỢC GỌI TOOL XÓA NGAY LẬP TỨC. AI phải DỪNG LẠI và hỏi lại để xác nhận CHÍNH XÁC câu sau: "Bạn có chắc chắn muốn xóa mục tiêu [Tên mục tiêu] không?". Bắt buộc phải chờ người dùng trả lời "Đồng ý" hoặc "Có" thì mới được gọi tool delete_goal.
4. QUY TẮC NẠP TIỀN: Cẩn thận kiểm tra giới hạn hũ khi thêm tiền.
5. CẬP NHẬT REALTIME DASHBOARD: Đảm bảo mọi thay đổi về tiền bạc từ Agent (sau khi thành công) đều được thông báo rõ cho người dùng biết là đã được cập nhật vào bảng Dashboard chính ngay lập tức.
6. THÀNH CÔNG: Khi đã làm xong lệnh, cứ báo "Xong! Em đã [hành động] rồi nhé 🎉. Đảm bảo mọi thay đổi về tiền bạc từ Agent đều được cập nhật vào bảng Dashboard chính ngay lập tức."
...
${goalsCtx}`;
    ...
    const tools = [{
      functionDeclarations: [
        { name: 'get_financial_status', description: 'Lấy trạng thái tài chính của User.' },
        { 
          ...
          parameters: {
            ...
            properties: {
              ...
              type: { type: SchemaType.STRING, description: 'EXPENSE hoặc INCOME' },
      ...
      for (const call of calls) {
        if (call.name === 'create_transaction') {
          ...
          this.alertsGateway?.sendAiActionAlert(userId, {
            ...
            summary: `Đã tự động cộng/trừ giao dịch "${args.title}" vào hũ.`,
          ...
          this.alertsGateway?.sendAiActionAlert(userId, {
            ...
            summary: `Giao dịch đã được cập nhật.`,
          ...
          actionsExecuted.push({ tool: call.name, args, result: 'success' });
           ...
           this.alertsGateway?.sendAiActionAlert(userId, {
              title: 'Cập nhật mục tiêu thành công!',
           ...
           const args = call.args as any;
           await this.prisma.savingsGoal.delete({ where: { id: args.goalId } });
           this.alertsGateway?.sendAiActionAlert(userId, {
              title: 'Xóa mục tiêu thành công!',
              summary: `Mục tiêu đã bị xóa.`,
    ...
    return { reply: result.response.text(), actionsExecuted };
  }
}