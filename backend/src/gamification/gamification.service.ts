...
export class GamificationService implements OnModuleInit {
  ...
  // User tự cấu hình mục tiêu tiết kiệm
  // Nhận deadline (ISO string) hoặc days
  async createCustomGoal(
    userId: string,
    dto: { title: string; targetAmount: number; deadline?: string; days?: number },
  ) {
    ...
    if (dto.deadline) {
      ...
      throw new BadRequestException('Cần truyền deadline (ISO date) hoặc days (số ngày).');
    }
